...
- `ipa()` phonemically transcribes words (real or not) in Portuguese or
  Spanish
...
- `stopwords_pt` and `stopwords_sp` contain stopwords in Portuguese and
  Spanish
...
to the function. Run `ipa_pt_test()` and `ipa_sp_test()` for sample
...
respects (`1`) or doesn’t repect (`0`) the SSP. In the example below,
...
- Garcia, G. D. (2017). _Weight effects on stress: Lexicon and grammar_
