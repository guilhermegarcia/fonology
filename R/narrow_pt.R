...
narrow_pt_vec <- function(word = "") {
  ...
  # Word-final /eN/ diphthongises (bem, homens). Elsewhere the vowel is only
  # nasalised and the nasal consonant is kept, as for the other vowels; the
  # rule used to drop the nasal and insert \u0272 (tempo -> te\u0303j\u0303\u0272.pu).
  narrow <- stringr::str_replace(narrow,
    pattern = "e[nm](s?)$",
    replacement = "e\u0303j\u0303\\1"
  )
  ...
  # Strong rhotic: broad transcription writes the phoneme /r/, narrow
  # transcription realises it as [x]. The tap is already /\u027e/ in broad
  # transcription and needs no adjustment.
  narrow <- stringr::str_replace_all(narrow,
    pattern = "r",
    replacement = "x"
  )
