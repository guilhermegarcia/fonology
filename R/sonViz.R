...
plotSon <- function(word = "", syl = FALSE, save_plot = FALSE) {
  ...
  affricates_vce <- "t\u0361s.d\u0361z.t\u0361\u0283.d\u0361\u0292" |>
    stringr::str_split("\\.") |>
    unlist()
  stops_vce <- "b.d.g.\u03b2.\u0256.\u0262" |>
    stringr::str_split("\\.") |>
    unlist()
  fricatives <- "\u0278.f.\u03b8.s.\u0283.\u0282.\u00e7.x.\u03c7.\u0127.h" |>
    stringr::str_split("\\.") |>
    unlist()
  affricates <- "t\u0361s.t\u0361\u0283" |>
    stringr::str_split("\\.") |>
  ...
  if (syl) {
    ...
    word_son <- tibble::tibble(phoneme = word_simple) |>
      ...
      dplyr::mutate(
        item = stringr::str_c("item", dplyr::row_number(), sep = "_"),
        syl = NA
      )
    ...
    sonPlot <- ggplot2::ggplot(data = word_son, ggplot2::aes(x = item, y = son)) +
      ...
      ggplot2::geom_label(
        data = word_son |> dplyr::filter(stress == 0),
        ggplot2::aes(
          label = phoneme, group = item,
          fill = syl
        ),
      ...
      # Stressed syllable:
      ggplot2::geom_label(
        data = word_son |> dplyr::filter(stress == 1),
        ggplot2::aes(
          label = phoneme, group = item,
          fill = syl
        ),
