...
#' @param lg Language. Currently, Portuguese (\code{pt}), French (\code{fr}) or Spanish (\code{sp}) are supported
...
ipa <- function(word, lg = "Portuguese", narrow = FALSE) {
  if (stringr::str_to_lower(lg) %in% c("pt", "portuguese")) {
    ...
    message("Only Portuguese, French and Spanish are currently supported.")
    return(NA)
  }
}
